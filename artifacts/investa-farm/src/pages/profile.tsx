...
// NotificationStatusRow removed from profile
...
export default function Profile() {
  ...
  const anyModalOpen = kycOpen || settingsOpen || walletOpen || rateOpen || currencyOpen || matcherOpen || pwaOpen || mfaOpen || demoOpen || pinSetupOpen;
  useEffect(() => {
  ...
  const { data: pinStatus, refetch: refetchPinStatus } = useQuery<{ hasPin: boolean }>({
    queryKey: ["pin-status"],
    queryFn: async () => {
      const r = await fetch("/api/wallet/pin/status", { headers: { Authorization: `Bearer ${token}` } });
      if (!r.ok) return { hasPin: false };
      return r.json();
    },
    enabled: !!token,
    staleTime: 30_000,
  ...
  const investorType = sessionStorage.getItem("investa_investor_type") ?? "individual";
  ...
  const { isDark, toggleDark: toggleTheme } = useTheme();
  ...
  const menuItems = [
    ...
    { icon: Film, label: "Product Demo", sublabel: "Watch all 6 feature walkthroughs", action: () => setDemoOpen(true), badge: null, badgeLabel: null },
  ];

  const DEMO_VIDEOS: Array<{ num: number; title: string; desc: string; color: string; icon: string; bg: string }> = [
    { num: 1, title: "Sign Up", desc: "Brand reveal · registration · OTP verification", color: "#16a34a", icon: "🔐", bg: "linear-gradient(135deg,#052e16,#14532d,#16a34a)" },
    { num: 2, title: "Browse Farms", desc: "Market listing · Kenyan farm cards", color: "#0284c7", icon: "🌾", bg: "linear-gradient(135deg,#0c4a6e,#0369a1,#38bdf8)" },
    { num: 3, title: "Invest in a Farm", desc: "Share selector · wallet payment · confirmation", color: "#d97706", icon: "📈", bg: "linear-gradient(135deg,#78350f,#b45309,#f59e0b)" },
    { num: 4, title: "Track Portfolio", desc: "Holdings · dividends · total returns", color: "#7c3aed", icon: "💼", bg: "linear-gradient(135deg,#2e1065,#5b21b6,#8b5cf6)" },
    { num: 5, title: "Top Up Wallet", desc: "M-Pesa STK push · success animation", color: "#15803d", icon: "💰", bg: "linear-gradient(135deg,#052e16,#166534,#22c55e)" },
    { num: 6, title: "Secondary Market", desc: "Listings · order book · trade confirmation", color: "#0891b2", icon: "⚡", bg: "linear-gradient(135deg,#164e63,#0e7490,#22d3ee)" },
  ];
  ...
  return (
    <div className="app-shell pb-20 page-enter" data-testid="profile-page">
      ...
      <div className="px-4 pt-4 space-y-3">
        {/* Wallet PIN setup banner — shown to users who haven't created a PIN yet */}
        {pinStatus !== undefined && !pinStatus?.hasPin && (
          <button onClick={() => setPinSetupOpen(true)}
            className="w-full rounded-2xl p-3.5 border border-amber-200 bg-amber-50 dark:bg-amber-950/30 dark:border-amber-800 flex items-center gap-3 text-left active:scale-98 transition-transform">
        ...
        {/* AI Smart Match card — the AI Agent now roams the app as a floating bubble instead of a static card here */}
        <button
          onClick={() => setMatcherOpen(true)}
          className="w-full rounded-2xl overflow-hidden text-left active:scale-[0.97] transition-transform"
        ...
        <LogoutConfirmDialog onConfirm={handleLogout}>
          <button data-testid="button-logout"
            className="w-full flex items-center gap-3 px-4 py-4 bg-red-50 dark:bg-red-950/40 border border-red-100 dark:border-red-900 rounded-2xl text-left active:scale-95 transition-transform">
            ...
            <span className="flex-1 text-red-500 dark:text-red-400 text-sm font-semibold">Sign Out</span>
            <ChevronRight size={15} className="text-red-300 dark:text-red-500" />
          </button>
        </LogoutConfirmDialog>
      ...
      {/* Wallet PIN setup / change sheet */}
      <WalletPinSetup
        open={pinSetupOpen}
        onClose={() => setPinSetupOpen(false)}
        onSuccess={() => { setPinSetupOpen(false); refetchPinStatus(); }}
        isFirstTime={!pinStatus?.hasPin}
      />
      ...
      <AnimatePresence>
        {settingsOpen && (
          <motion.div className="fixed inset-0 z-50 flex items-end justify-center"
            ...
            <motion.div className="relative w-full max-w-[430px] bg-background rounded-t-3xl pb-10 flex flex-col"
      ...
      {/* ── Video Player Modal ── */}
      <AnimatePresence>
        {videoOpen && activeVideoNum && (
          <motion.div className="fixed inset-0 z-[70] flex items-center justify-center"
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <div className="absolute inset-0 bg-black/80 backdrop-blur-sm"
              onClick={() => { setVideoOpen(false); setActiveVideoNum(null); }} />
            <motion.div className="relative w-full max-w-[430px] mx-auto flex flex-col"
              style={{ height: "90dvh" }}
              initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              transition={{ type: "spring", damping: 28, stiffness: 300 }}>
              <div className="flex items-center justify-between px-4 py-3 bg-background rounded-t-2xl border-b border-border flex-shrink-0">
                <p className="font-bold text-sm text-foreground">
                  {DEMO_VIDEOS.find(v => v.num === activeVideoNum)?.title ?? "Product Demo"}
                </p>
                <button onClick={() => { setVideoOpen(false); setActiveVideoNum(null); }}
                  className="w-8 h-8 rounded-full bg-muted flex items-center justify-center">
                  <X size={15} className="text-muted-foreground" />
                </button>
              </div>
              <div className="flex-1 rounded-b-2xl overflow-hidden">
                <InlineVideoPlayer key={activeVideoNum} videoNum={activeVideoNum} />
              </div>
            </motion.div>
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {demoOpen && (
          <motion.div className="fixed inset-0 z-[60] flex items-end justify-center"
            ...
            <motion.div className="relative w-full max-w-[430px] bg-background rounded-t-3xl shadow-2xl overflow-hidden"
              initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }}
              transition={{ type: "spring", damping: 30, stiffness: 300 }}>
              ...
              <div className="flex items-center justify-between px-5 py-3 border-b border-border">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="font-bold text-sm">Product Demo</p>
                    <p className="text-muted-foreground text-[10px]">6 feature walkthroughs</p>
                  </div>
                </div>
                <button onClick={() => setDemoOpen(false)} className="w-8 h-8 rounded-full bg-muted flex items-center justify-center">
                  <X size={15} className="text-muted-foreground" />
                </button>
              </div>

              {/* Video list */}
              <div className="px-4 py-4 space-y-2.5 max-h-[72dvh] overflow-y-auto pb-10">
                {DEMO_VIDEOS.map(({ num, title, desc, bg, icon, color }) => (
                  <button key={num}
                    onClick={() => {
                      setActiveVideoNum(num as 1|2|3|4|5|6);
                      setVideoOpen(true);
                      setDemoOpen(false);
                    }}
                    className="w-full flex items-center gap-3 p-3 bg-card border border-border rounded-2xl active:scale-[0.98] transition-transform text-left overflow-hidden">
                    {/* Thumbnail — mini screenshot preview */}
                    <div className="relative w-[72px] h-[46px] rounded-xl flex-shrink-0 overflow-hidden flex items-center justify-center"
                      style={{ background: bg }}>
                      {/* Mini phone chrome */}
                      ...
                      <div className="absolute top-1 left-1 bg-black/40 rounded px-1 py-0.5">
                        <span className="text-white text-[8px] font-black">{num}</span>
                      </div>
                      {/* Play overlay */}
                      <div className="absolute inset-0 flex items-center justify-center bg-black/0 hover:bg-black/20 transition-colors">
                        <div className="w-5 h-5 rounded-full bg-white/70 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
                          <Play size={9} className="ml-0.5" style={{ color }} fill={color} />
                        </div>
                      </div>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-foreground text-sm font-semibold leading-tight">{title}</p>
                      <p className="text-muted-foreground text-[10px] mt-0.5 leading-snug">{desc}</p>
                    </div>
                    <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0"
                      style={{ background: `${color}18` }}>
                      <Play size={13} className="ml-0.5" style={{ color }} fill={color} />
                    </div>
                  </button>
                ))}
                <p className="text-center text-muted-foreground text-[10px] pt-1">
                  Plays in-app · best viewed on mobile width
                </p>
              </div>
            </motion.div>
          </motion.div>
