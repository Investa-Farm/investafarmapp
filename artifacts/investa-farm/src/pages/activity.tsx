...
import { TrendingUp, TrendingDown, ArrowUpRight, Clock, Receipt, X, ChevronRight, Calendar, Layers, DollarSign, CheckCircle2, Newspaper, MapPin, Share2, Copy, Check, ShieldCheck, Fingerprint, Trash2, Download } from "lucide-react";
...
function ReceiptSheet({ tx, onClose, onDelete, deleting }: { tx: TxItem; onClose: () => void; onDelete: (id: number) => void; deleting: boolean }) {
      ...
      >
        ...
        <div className="px-5 pb-8 pt-1">
          ...
          <button
            ...
            disabled={deleting}
            className="w-full mt-2.5 py-3 rounded-2xl border-2 border-red-100 text-red-500 font-bold text-sm active:scale-95 transition-transform flex items-center justify-center gap-1.5 disabled:opacity-50"
          >
            <Trash2 size={14} /> {deleting ? "Removing…" : "Delete from history"}
          </button>
        </div>
      </motion.div>
    </div>
...
export default function Activity() {
  ...
  const token = typeof window !== "undefined" ? sessionStorage.getItem("investa_token") : null;
  const qc = useQueryClient();

  const deleteTxMutation = useMutation({
    mutationFn: async (id: number) => {
      const r = await fetch(`/api/transactions/${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
      if (!r.ok) throw new Error("Failed to delete transaction");
    },
    onSuccess: (_data, id) => {
      qc.setQueryData<TxItem[]>(["/api/transactions"], (old) => (old ?? []).filter(t => t.id !== id));
  ...
  const exportTransactionsCsv = () => {
    downloadCsv(
      `investa-farm-transactions-${new Date().toISOString().slice(0, 10)}`,
      ["Date", "Time", "Type", "Farm", "Crop", "Quantity", "Price Per Share (KES)", "Total Amount (KES)", "Exit Type", "Status", "Reference"],
        ...
        <div className="flex items-start justify-between">
          ...
          {tab === "transactions" && txCount > 0 && (
            <button
              ...
              className="flex items-center gap-1.5 bg-white/15 rounded-full px-3 py-1.5 mt-0.5 active:scale-95 transition-transform"
            >
              <Download size={12} className="text-white" />
              <span className="text-white text-[11px] font-semibold">CSV</span>
            </button>
          )}
        </div>
        {txCount > 0 && (
            ...
            : transactions?.length === 0
              ...
              : transactions?.map((tx: any) => {
                  ...
                  return (
                    ...
                    >
                      <button className="flex items-center gap-3 flex-1 min-w-0 text-left" onClick={() => setSelectedTx(tx as TxItem)}>
                      ...
                      <div className="flex items-center gap-1.5 flex-shrink-0">
                        ...
                        <button
                          onClick={(e) => { e.stopPropagation(); if (confirm("Remove this transaction from your history?")) deleteTxMutation.mutate(tx.id); }}
      ...
      <AnimatePresence>
        {selectedTx && (
          <ReceiptSheet
            ...
            onDelete={(id) => { if (confirm("Remove this transaction from your history?")) deleteTxMutation.mutate(id); }}
            deleting={deleteTxMutation.isPending}
          />
        )}
      </AnimatePresence>
