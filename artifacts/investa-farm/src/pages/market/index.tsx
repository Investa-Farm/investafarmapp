...
function InvestorChecklist({ walletBalance }: { walletBalance?: string }) {
  ...
  if (doneCount === steps.length) {
    // Show a "you're all set" state instead of silently disappearing
    return (
      <div className="mx-4 mt-3 bg-gradient-to-r from-[#052e16] to-[#166534] rounded-2xl px-4 py-3 flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-green-200 text-[11px] mt-0.5">Profile complete · KYC approved · Ready to invest</p>
        </div>
        <button onClick={() => { localStorage.setItem("investa_checklist_dismissed", "1"); setDismissed(true); }}
    ...
    <div className="app-shell pb-20 page-enter" data-testid="market-home">
      ...
      <div className="px-4 pt-4 space-y-5">
        {activeSection === "market" && (
          <>
            ...
            <Link href="/market/portfolios">
              <div className="mt-8 mb-2 bg-muted/50 border border-border rounded-2xl p-3.5 flex items-center gap-3 cursor-pointer active:scale-[0.98] transition-all">
        ...
        {activeSection === "watchlist" && (
          <section className="space-y-3">
            {/* Watchlist header */}
            <div className="rounded-2xl overflow-hidden relative" style={{ background: "linear-gradient(145deg, #052e16 0%, #14532d 45%, #166534 100%)" }}>
              <div className="absolute inset-0 opacity-10" style={{ backgroundImage: "radial-gradient(circle at 90% 20%, #4ade80 0%, transparent 55%)" }} />
              <div className="relative p-4">
                <div className="flex items-start justify-between">
                  <div>
                    <span className="text-[9px] font-black uppercase tracking-[0.2em] text-green-400/70">Season Watchlist</span>
                    <h2 className="text-white font-extrabold text-lg leading-tight mt-0.5">Upcoming Crops</h2>
                    <p className="text-white/50 text-[11px] mt-0.5">Save crops &amp; commit capital early</p>
                  </div>
                  <div className="flex gap-2">
                    <div className="bg-white/10 border border-white/10 rounded-2xl px-3 py-2 text-center">
                      <p className="text-white/50 text-[8px] uppercase tracking-wider font-bold">Watching</p>
                      <p className="text-green-300 font-black text-xl leading-none mt-0.5">{watchlisted.size}</p>
                    </div>
                    {Object.values(committed).reduce((a, b) => a + b, 0) > 0 && (
                      <div className="bg-white/10 border border-white/10 rounded-2xl px-3 py-2 text-center">
                        <p className="text-white/50 text-[8px] uppercase tracking-wider font-bold">Committed</p>
                        <p className="text-green-400 font-black text-sm leading-none mt-0.5">{formatKES(Object.values(committed).reduce((a, b) => a + b, 0))}</p>
                      </div>
                    )}
                  </div>
                </div>
                {watchlisted.size === 0 && (
                  <div className="mt-3 pt-3 border-t border-white/10 flex items-center gap-2">
                    <BookmarkPlus size={11} className="text-white/35" />
                    <span className="text-white/40 text-[10px] font-medium">Tap "Watch" on any crop card to track it here</span>
                  </div>
                )}
              </div>
            </div>
              ...
              return (
                <div key={crop.id}
                  className={`bg-card rounded-2xl border overflow-hidden transition-all ${
                    isWatchlisted ? "border-primary/30 shadow-md shadow-green-500/10" : "border-border"
                  }`}
                >
                  {/* Active watch indicator */}
                  {isWatchlisted && (
                    <div className="h-0.5 w-full bg-gradient-to-r from-primary/40 via-green-400 to-primary/40" />
                  )}

                  {/* Image hero */}
                  <div className="relative h-36">
                    <img src={crop.image} alt={crop.name} className="w-full h-full object-cover" />
                    <div className="absolute inset-0" style={{ background: "linear-gradient(135deg,rgba(0,0,0,0.75) 0%,rgba(0,0,0,0.45) 55%,rgba(0,0,0,0.15) 100%)" }} />
                    <div className="absolute inset-0 p-3.5 flex flex-col justify-between">
                      {/* Top row */}
                      <div className="flex items-start justify-between">
                        <div>
                          <p className="text-white font-extrabold text-xl leading-none tracking-tight">{crop.name}</p>
                          <p className="text-white/60 text-[10px] font-medium mt-0.5">{crop.season}</p>
                        </div>
                        <div className="bg-black/35 backdrop-blur-sm rounded-xl px-2.5 py-1.5 text-right border border-white/10 flex-shrink-0">
                          <p className="text-white/50 text-[7px] uppercase tracking-widest font-bold">Est. ROI</p>
                          <p className="text-green-300 font-black text-base leading-tight">{crop.expectedReturn}</p>
                        </div>
                      </div>
                      {/* Bottom badges */}
                      <div className="flex items-center gap-1.5 flex-wrap">
                        {isWatchlisted && (
                          <span className="bg-primary/90 text-white text-[8px] font-bold px-2 py-0.5 rounded-full tracking-wide flex items-center gap-1">
                            <span className="w-1.5 h-1.5 rounded-full bg-green-300 inline-block" />Watching
                          </span>
                        )}
                        <span className={`text-[8px] font-bold px-2 py-0.5 rounded-full ${crop.demandColor}`}>{crop.demand} Demand</span>
                        <span className="bg-white/15 text-white/80 text-[8px] font-medium px-1.5 py-0.5 rounded-full">{crop.farms} farm{crop.farms !== 1 ? "s" : ""}</span>
                  ...
                  <div className="grid grid-cols-3 border-b border-border/70">
                    {[
                      { label: "Planting", value: crop.plantingStart },
                      { label: "Harvest",  value: crop.harvestEst },
                      { label: "Target",   value: crop.expectedReturn, highlight: true },
                    ].map(({ label, value, highlight }) => (
                      <div key={label} className={`py-2.5 text-center ${highlight ? "bg-primary/5" : ""} border-r border-border/70 last:border-r-0`}>
                        <p className="text-muted-foreground/60 text-[7px] uppercase font-black tracking-[0.15em]">{label}</p>
                        <p className={`font-extrabold text-[11px] mt-0.5 ${highlight ? "text-primary" : "text-foreground"}`}>{value}</p>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="p-3">
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl border border-amber-200 bg-amber-50 text-amber-700 text-xs font-bold transition-all active:scale-95 hover:bg-amber-100">
                        <Lightbulb size={12} />
                        Insight
                      </button>
                      ...
                      {committed[crop.id] ? (
                        <button
                          ...
                          className="flex-1 flex items-center justify-center gap-1.5 bg-green-600 text-white text-xs font-bold py-2.5 rounded-xl active:scale-95 transition-transform shadow-sm shadow-green-600/20">
                        ...
                        <button
                          ...
                          className="flex-1 flex items-center justify-center gap-1.5 bg-foreground text-background text-xs font-bold py-2.5 rounded-xl active:scale-95 transition-transform">
                          <DollarSign size={12} />
                          Commit Funds
                        </button>
                      )}
