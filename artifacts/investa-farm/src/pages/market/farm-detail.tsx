...
export default function FarmDetail() {
  ...
  useScrollLock(comparePickerOpen);
  ...
  return (
    <div ref={scrollRef} className="app-shell pb-56 page-enter overflow-y-auto h-screen" data-testid="farm-detail">
        ...
        >
            ...
            <>
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <div className="rounded-2xl p-4 bg-gradient-to-br from-orange-50 to-amber-50 border border-orange-200">
                  ...
                  <p className="text-orange-500 font-black text-2xl" style={{ fontFamily: "Space Grotesk, sans-serif" }}>
                    +{Math.round(Math.max(6, 9 + (farm.changePercent ?? 0) * 0.3))}%
                  </p>
                  <p className="text-orange-400 text-[10px] mt-0.5">30–60 days · est.</p>
                </div>
                <div className="rounded-2xl p-4 bg-gradient-to-br from-emerald-50 to-green-50 border border-[#16a34a]/25">
                  ...
                  <p className="text-[#16a34a] font-black text-2xl" style={{ fontFamily: "Space Grotesk, sans-serif" }}>
                    +{Math.round(Math.max(14, 18 + (farm.changePercent ?? 0) * 0.6))}%
                  </p>
                  <p className="text-[#16a34a]/60 text-[10px] mt-0.5">~6 months · est.</p>
                </div>
              </div>
              ...
              {chartData.length > 0 && (
                <div className="bg-card rounded-2xl border border-border p-4">
                  ...
                  <div className="grid grid-cols-3 gap-2 mt-3">
                    {[
                      { label: "52w Low (est.)",  val: formatKES(Math.round(farm.currentPrice * 0.88)), color: "text-red-500" },
                      { label: "Current",         val: formatKES(farm.currentPrice),                   color: "text-foreground" },
                      { label: "52w High (est.)", val: formatKES(Math.round(farm.currentPrice * 1.18)), color: "text-[#16a34a]" },
                    ].map(({ label, val, color }) => (
          ...
          {activeTab === "farm-health" && (
            <>
              ...
              {/* Pricing engine v2: revenue confidence interval + risk explainability */}
              {(farm.revenueForecast || (farm.topFactors && farm.topFactors.length > 0)) && (
                <div className="bg-card border border-border rounded-2xl p-4">
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <Scale size={14} className="text-[#16a34a]" />
                      <p className="text-sm font-bold">Revenue Forecast &amp; Risk Factors</p>
                    </div>
                    {farm.reviewFlagged && (
                  ...
                  {farm.revenueForecast && (
                    <div className="mb-4">
                      <div className="flex items-baseline justify-between mb-1.5">
                        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Expected harvest revenue</p>
                        <p className="text-[10px] text-muted-foreground">±{Math.round((farm.revenueForecast.uncertaintyRatio ?? 0) * 50)}%</p>
                      </div>
                      <div className="relative h-2.5 bg-muted rounded-full overflow-hidden mb-1.5">
                        <div className="absolute inset-y-0 left-0 right-0 rounded-full" style={{ background: "linear-gradient(90deg, rgba(22,163,74,0.15), rgba(22,163,74,0.5), rgba(22,163,74,0.15))" }} />
                      </div>
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-muted-foreground">{formatKES(farm.revenueForecast.low)}</span>
                        <span className="font-bold text-[#16a34a]">{formatKES((farm.revenueForecast.low + farm.revenueForecast.high) / 2)}</span>
                        <span className="text-muted-foreground">{formatKES(farm.revenueForecast.high)}</span>
                      </div>
                      <p className="text-muted-foreground text-[10px] mt-1.5">Low / expected / high range — wider bands mean less certainty about this harvest's payout.</p>
                    </div>
                  )}

                  {farm.topFactors && farm.topFactors.length > 0 && (
                    <div className="space-y-2">
                      <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">
                        Factors behind risk score {farm.riskScore ? `(${farm.riskScore}/10)` : ""}
                        {farm.riskScoreSource === "groq" && <span className="ml-1.5 text-[#16a34a]">· AI-scored</span>}
                      </p>
                      {farm.topFactors.map((f, i) => (
                        <div key={i} className="flex items-start justify-between gap-2 text-xs">
                          <span className="text-muted-foreground flex-1">{f.detail ?? f.factor.replace(/_/g, " ")}</span>
                          <span className={`font-bold whitespace-nowrap ${f.contribution >= 0 ? "text-[#16a34a]" : "text-red-500"}`}>
                  ...
                  {farm.coldStart && (
                    <p className="text-amber-600 text-[10px] mt-3 bg-amber-50 rounded-lg px-2.5 py-1.5">
                      First-time farmer listing — risk score is capped until a harvest completes successfully.
                    </p>
                  )}
                </div>
              )}
      ...
      {listing && (
        <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[430px] z-50 pointer-events-none">
          <div className="pointer-events-auto bg-background/95 backdrop-blur-xl border-t border-border px-4 pt-3 pb-8 shadow-2xl">
            <div className="flex items-center justify-between mb-3">
              <div>
                <p className="text-[10px] text-muted-foreground font-semibold uppercase tracking-wide">Price per Share</p>
                <p className="text-xl font-black text-foreground" style={{ fontFamily: "Space Grotesk, sans-serif" }}>{formatKES(listing.pricePerShare)}</p>
              </div>
              <div className="text-right">
                <p className="text-[10px] text-muted-foreground font-semibold uppercase tracking-wide">Available</p>
                <p className="text-xl font-black text-foreground" style={{ fontFamily: "Space Grotesk, sans-serif" }}>{listing.sharesAvailable.toLocaleString()} <span className="text-xs font-semibold text-muted-foreground">shares</span></p>
              </div>
            </div>
            <button
      ...
      <ShareModal
        ...
        title={farm.name}
        text={`🌱 Invest in ${farm.name} on Investa Farm! ${farm.cropType} · ${farm.location}`}
