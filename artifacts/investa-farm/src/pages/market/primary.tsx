...
import { ArrowLeft, TrendingUp, TrendingDown, Search, SlidersHorizontal, ChevronDown, ChevronUp, Zap, X, Users, Bell, Bookmark, BookmarkCheck, ListChecks, ListPlus, Check } from "lucide-react";
...
import { InvestListDrawer } from "@/components/invest-list-drawer";
...
type Listing = {
  ...
  totalShares?: number; changePercent: number; imageUrl?: string;
};

/** Real funding progress bar driven by sharesAvailable / totalShares from the API */
function FundingProgress({ sharesAvailable, totalShares }: { sharesAvailable: number; totalShares?: number }) {
  const total = totalShares ?? 10_000;
  const sold = Math.max(0, total - sharesAvailable);
  const pct = total > 0 ? Math.min(100, (sold / total) * 100) : 0;
  const pctLabel = pct.toFixed(0);
  const isNearFull = pct >= 80;
  const color = isNearFull ? "#dc2626" : pct >= 50 ? "#d97706" : "#16a34a";

  return (
    <div className="flex items-center gap-1.5 min-w-0">
      {/* thin progress bar */}
      <div className="relative h-1.5 w-16 rounded-full bg-muted overflow-hidden flex-shrink-0 border border-border">
        <motion.div
      ...
      <span className="text-[9px] font-black tabular-nums" style={{ color }}>{pctLabel}%</span>
      <span className="text-[8px] font-semibold text-muted-foreground">funded</span>
    </div>
  );
}
...
export default function PrimaryMarket() {
  ...
  const { add: addToInvestList, isInList, items: investListItems } = useInvestList();
  const [investListOpen, setInvestListOpen] = useState(false);
  ...
  return (
    ...
    >
      ...
      <div className="bg-background border-b border-border">
        <div className="flex items-center gap-3 px-4 pt-12 pb-3">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setInvestListOpen(true)}
              className="relative w-8 h-8 rounded-lg flex items-center justify-center active:scale-90 transition-transform bg-muted border border-border">
              <ListChecks size={13} className="text-muted-foreground" />
              {investListItems.length > 0 && (
                <span className="absolute -top-1 -right-1 min-w-[15px] h-[15px] px-0.5 rounded-full bg-primary text-white text-[8px] font-black flex items-center justify-center">
                  {investListItems.length}
                </span>
              )}
            </button>
                      ...
                      <div className="flex flex-col gap-3 items-stretch">
                        {listing.sharesAvailable > 0 ? (
                          <button
                            ...
                            className="px-2.5 py-1.5 rounded-lg text-[10px] font-black tracking-wide text-white active:scale-90 transition-transform shadow-sm w-full bg-primary hover:bg-primary/90">
                            BUY
                          </button>
                        ) : (
                          <span className="px-2.5 py-1.5 rounded-lg text-[9px] font-black tracking-wide text-white/90 shadow-sm w-full bg-muted-foreground/60 text-center">
                            FUNDED
                          </span>
                        )}
                        {listing.sharesAvailable > 0 && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              addToInvestList({
                                listingId: listing.id, farmId: listing.farmId, farmName: listing.farmName,
                                cropType: listing.cropType, pricePerShare: listing.pricePerShare,
                                sharesAvailable: listing.sharesAvailable, imageUrl: listing.imageUrl,
                              }, 10);
                            }}
                            className={`px-2 py-1 rounded-lg text-[8px] font-bold tracking-wide flex items-center justify-center gap-0.5 w-full border transition-transform active:scale-90 ${
                              isInList(listing.id) ? "bg-primary/10 text-primary border-primary/30" : "bg-muted text-muted-foreground border-border"
                            }`}>
                            {isInList(listing.id) ? <Check size={9} /> : <ListPlus size={9} />}
                            {isInList(listing.id) ? "Added" : "Add"}
                          </button>
                        )}
                      </div>
                    </div>
                    ...
                    <button onClick={() => setExpandedId(isExpanded ? null : listing.id)}
                      ...
                      <div className="flex-1 flex items-center gap-2">
                        ...
                        <FundingProgress
                          ...
                          totalShares={(listing as any).totalShares}
                        />
                      </div>
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="flex items-center gap-0.5 text-muted-foreground text-[9px] font-semibold ml-2">
                          ...
                          <div className="px-4 py-4 space-y-4 bg-muted/20">
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <span className={`text-[9px] font-black px-2 py-0.5 rounded-full border ${
                                ...
                                {listing.sharesAvailable <= 0 ? "🎉 Fully Funded" : `${listing.sharesAvailable.toLocaleString()} shares left`}
                              </span>
                            </div>
      ...
      <InvestListDrawer open={investListOpen} onClose={() => setInvestListOpen(false)} />
