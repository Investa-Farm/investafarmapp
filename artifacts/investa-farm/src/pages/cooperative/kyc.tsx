...
const DOC_TYPES = [
  {
    value: "national_id",
    label: "Chairman's National ID (Front)",
    emoji: "🪪",
    hint: "Front side of the cooperative chairman or director's national ID",
    required: true,
    category: "identity",
  },
  {
    value: "national_id_back",
    label: "Chairman's National ID (Back)",
    emoji: "🪪",
    hint: "Back side of the chairman or director's national ID",
    required: true,
    category: "identity",
  },
  {
    ...
    emoji: "🤳",
    hint: "Chairman/director holding their national ID — face must be clearly visible",
    required: true,
    category: "identity",
  },
  {
    ...
    emoji: "📋",
    hint: "Cooperative/SACCO registration certificate from the Registrar of Cooperatives or relevant authority",
    required: true,
    category: "org",
  },
  {
    ...
    emoji: "💰",
    hint: "Bank statement or audited accounts for the last 3 months",
    required: true,
    category: "org",
  },
  {
    ...
    try {
      ...
      const kycRes = await fetch("/api/kyc/upload", {
        ...
        body: JSON.stringify({
          ...
          fileUrl,
          notes: `Uploaded by ${user?.name ?? "user"} for cooperative KYC verification`,
        }),
      });
      ...
      qc.invalidateQueries({ queryKey: ["coop-kyc-docs"] });
      await refetch();
      setSubmitDone(true);
              ...
              <div className="px-5 pb-10 pt-2">
                <div className="flex items-center justify-between mb-5">
                  <div className="flex items-center gap-3">
                    <div className="w-11 h-11 rounded-2xl bg-primary/10 flex items-center justify-center text-2xl">{doc.emoji}</div>
                    <div>
                ...
                <div className="space-y-3">
                  ...
                  {stagedEntry && (
                    <button onClick={() => handleUpload(activeUploadType)} disabled={isUp}
...
function DocCard({
  doc, kycDoc, staged, isUploading, onView, onOpenSheet,
}: {
      ...
      <div className="p-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap mb-0.5">
              ...
              {isRejected && (
                <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-red-100 text-red-700">Re-upload Needed</span>
              )}
            </div>
        ...
        {kycDoc && !isRejected && (
          <div className="mt-3 bg-background rounded-xl border border-border p-2.5 flex items-center gap-2">
            ...
            {kycDoc.fileUrl && (
              <button onClick={() => onView(kycDoc)} className="flex items-center gap-1 text-primary text-[10px] font-bold flex-shrink-0">
                <Eye size={11} /> View
              </button>
            )}
          </div>
        ...
        {(!uploaded || isRejected) && (
          <button onClick={() => onOpenSheet(doc.value)}
