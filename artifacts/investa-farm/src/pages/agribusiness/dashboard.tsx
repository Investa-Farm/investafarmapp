...
export default function AgribusinessDashboard() {
  ...
  return (
    <div className="app-shell page-enter" style={{ display: "flex", flexDirection: "column", height: "100dvh", overflow: "hidden" }}>
      ...
      {kycUnderReview && (
        <div className="mx-4 mt-2 mb-0 bg-blue-50 border border-blue-200 rounded-2xl px-4 py-3 space-y-2.5">
              ...
              <div key={step.label} className="flex items-center flex-1">
                <div className="flex flex-col items-center flex-1">
                  <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold ${
                    step.done ? "bg-blue-500 text-white" : "bg-blue-100 text-blue-400 border border-blue-200"
                  } ${step.active ? "ring-2 ring-blue-300 ring-offset-1" : ""}`}>
                    {step.done ? "✓" : i + 1}
                  </div>
                  <p className={`text-[9px] mt-0.5 font-medium ${step.done ? "text-blue-700" : "text-blue-300"}`}>{step.label}</p>
                </div>
                {i < 2 && <div className={`h-0.5 flex-1 mx-0.5 mb-3 ${step.done ? "bg-blue-400" : "bg-blue-100"}`} />}
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="relative z-10 pt-12 px-5">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={toggleDark} className="w-9 h-9 rounded-full bg-white/20 border border-white/30 flex items-center justify-center" title={isDark ? "Switch to light mode" : "Switch to dark mode"}>
