...
import {
  ArrowLeft, Eye, EyeOff, Loader2, Lock, Mail, TrendingUp,
  User, Phone, CheckSquare, ShieldCheck, Smartphone, Linkedin,
} from "lucide-react";
...
export default function InvestorAuth() {
  ...
  function handleGoogleAuth() {
    window.location.href = "/api/auth/google?role=investor";
  }

  function handleLinkedInAuth() {
    window.location.href = "/api/auth/linkedin?role=investor";
  }
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    login.mutate({ data: { email, password } }, {
      onSuccess: (data: any) => {
        ...
        setToken(data.token); storeUser(data.user); setLocation("/market");
      },
      onError: (err: any) => {
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    const country = PHONE_CODES.find(c => c.code === phoneCode)?.name;
    register.mutate({ data: { email, password, name, role: "investor", ...(fullPhone ? { phone: fullPhone } : {}), ...(country ? { country } : {}) } as any }, {
      onSuccess: (data: any) => {
  ...
  const handleTypeSelected = (_type: InvestorType) => {
    sessionStorage.setItem("investa_investor_type", _type);
    if (_type === "fund_manager") setLocation("/wealth");
    ...
    return (
      <div className="min-h-dvh w-full max-w-[430px] mx-auto flex flex-col items-center justify-center p-8"
        ...
        <AnimatePresence mode="wait">
          <motion.div key={welcomeIdx} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }}
        ...
        <div className="flex gap-1.5 justify-center mt-8">
          {WELCOME_STEPS.map((_, i) => (
            <motion.div key={i} animate={{ width: i === welcomeIdx ? 24 : 6 }}
              className={`h-1.5 rounded-full ${i === welcomeIdx ? "bg-white" : "bg-white/30"}`} />
          ))}
        </div>
        <motion.button whileTap={{ scale: 0.97 }}
          onClick={() => { if (welcomeIdx < WELCOME_STEPS.length - 1) setWelcomeIdx(n => n + 1); else setStep("investor-type"); }}
        ...
        {welcomeIdx < WELCOME_STEPS.length - 1 && (
          <button onClick={() => setStep("investor-type")} className="text-white/50 text-xs underline mt-3">Skip intro</button>
        )}
      </div>
    );
  }
    ...
    <>
      <div className="min-h-dvh w-full max-w-[430px] mx-auto flex flex-col" data-testid="investor-auth">
        ...
        <div className="relative pt-14 pb-10 px-6 flex flex-col items-center gap-4 overflow-hidden"
          style={{ background: "linear-gradient(160deg, #0a2e14 0%, #14532d 45%, #16a34a 80%, #22c55e 100%)" }}>
          <img src={investorBg} alt="" className="absolute inset-0 w-full h-full object-cover opacity-15 mix-blend-luminosity" />
          {/* Decorative orbs */}
          <div className="absolute -top-10 -right-10 w-48 h-48 rounded-full pointer-events-none"
            style={{ background: "radial-gradient(circle, rgba(74,222,128,0.2) 0%, transparent 70%)" }} />
          <div className="absolute bottom-0 -left-6 w-36 h-36 rounded-full pointer-events-none"
            style={{ background: "radial-gradient(circle, rgba(22,163,74,0.25) 0%, transparent 70%)" }} />
          {/* Floating particles */}
          <Particle x="12%" y="20%" size={6} dur={3.2} />
          <Particle x="80%" y="15%" size={4} dur={2.8} />
          <Particle x="60%" y="55%" size={5} dur={3.8} />
          <Particle x="25%" y="65%" size={3} dur={2.5} />
          ...
          <div className="relative z-10 w-full">
            ...
            <div className="flex flex-col items-center gap-4">
              <motion.div initial={{ scale: 0.7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
                ...
                className="relative">
                {/* Pulse ring */}
                <motion.div animate={{ scale: [1, 1.18, 1], opacity: [0.3, 0.12, 0.3] }}
                ...
                <div className="w-20 h-20 rounded-2xl border border-white/25 flex items-center justify-center"
                  ...
                  <img src={logoSrc} alt="Investa Farm" className="h-11 w-auto" style={{ filter: "brightness(0) invert(1)" }} />
                </div>
              </motion.div>

              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
                className="text-center">
                <h1 className="text-white text-2xl font-bold tracking-tight">Investor Portal</h1>
              ...
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
                ...
                {[["🌾","10–28%\nReturns"],["📊","Live\nMarket"],["🏆","Verified\nFarms"]].map(([e,l]) => (
                  <div key={l} className="rounded-xl px-3 py-2 border border-white/20 text-center"
                    ...
                    <p className="text-white/70 text-[9px] whitespace-pre font-medium">{l}</p>
                  </div>
                ))}
              </motion.div>
            </div>
              ...
              {tab === "login" ? (
                <form onSubmit={handleLogin} className="space-y-4">
                  ...
                  <button type="button" onClick={() => {
                    const demoEmail = "david.investor@investafarm.com";
                    const demoPass  = "password123";
                    setEmail(demoEmail); setPassword(demoPass); setError("");
                    login.mutate({ data: { email: demoEmail, password: demoPass } }, {
                      onSuccess: (data: any) => {
                        if (data.totpRequired) { setTempToken(data.tempToken ?? ""); setTotpStep(true); return; }
                        if (!data.user) { setError("Demo login failed"); return; }
                        if (data.user.role !== "investor") { setError("Not an investor account"); return; }
                        setToken(data.token); storeUser(data.user); setLocation("/market");
                      },
                      onError: (err: any) => {
                        if (err?.data?.requiresOtp) { setToken(err.data.token); storeUser(err.data.user); setLocation(`/verify-otp?email=${encodeURIComponent(err.data.email ?? "david.investor@investafarm.com")}`); return; }
                  ...
                  <p className="text-center text-gray-400 text-xs pt-1">
                    New here?{" "}
                    <button type="button" onClick={() => setTab("register")} className="text-primary font-semibold">Create account</button>
                  </p>
                </form>
              ) : (
                <form onSubmit={handleRegister} className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex gap-2">
                      <select value={phoneCode} onChange={e => setPhoneCode(e.target.value)}
                        className="border-2 border-gray-200 rounded-xl px-2 py-3 text-sm bg-gray-50 focus:outline-none focus:border-primary appearance-none w-[88px] flex-shrink-0 text-center font-semibold">
                        {PHONE_CODES.map(c => <option key={c.code} value={c.code}>{c.name} ({c.code})</option>)}
                      </select>
                      <div className="relative flex-1">
                        <Phone size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400" />
                        <input type="tel" value={phone} onChange={e => setPhone(e.target.value)}
                          placeholder={phoneCode === "+254" ? "7XX XXX XXX" : "Phone number"}
                          className="w-full border-2 border-gray-200 rounded-xl pl-10 pr-4 py-3 text-gray-900 bg-gray-50 text-sm focus:outline-none focus:border-primary focus:bg-white transition-colors" />
                      </div>
                    </div>
                  </div>
                  {/* Investment range */}
                  <div className="space-y-1.5">
                    <label className="text-gray-400 text-xs font-semibold uppercase tracking-wider">Investment Range (KES)</label>
                    <div className="grid grid-cols-3 gap-2">
                      {["5K–50K","50K–500K","500K+"].map(r => (
                        <motion.button key={r} type="button" whileTap={{ scale: 0.95 }} onClick={() => setInvestAmount(r)}
                          className={`py-2.5 rounded-xl border-2 text-xs font-semibold transition-all ${investAmount === r ? "border-primary bg-green-50 text-primary" : "border-gray-200 text-gray-400"}`}>
                          {r}
                        </motion.button>
                      ))}
                    </div>
                  ...
                  <label className="flex items-start gap-3 cursor-pointer">
                    <motion.button type="button" whileTap={{ scale: 0.9 }} onClick={() => setAgreed(a => !a)}
                      className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 mt-0.5 transition-colors ${agreed ? "bg-primary border-primary" : "border-gray-300"}`}>
                  ...
                  <p className="text-center text-gray-400 text-xs pt-1">
                    ...
                    <button type="button" onClick={() => setTab("login")} className="text-primary font-semibold">Sign in</button>
                  </p>
                </form>
              )}
            </motion.div>
    ...
    <div className="space-y-1.5">
      ...
      <div className="relative">
        ...
        <input data-testid={`input-${id}`} type={type} value={value} onChange={e => set(e.target.value)}
          ...
          className={`w-full border-2 border-gray-200 rounded-xl py-3 text-gray-900 bg-gray-50 text-sm focus:outline-none focus:border-primary focus:bg-white transition-all ${icon ? "pl-10 pr-4" : "px-4"}`} />
      </div>
    </div>
  );
}
    ...
    <div className="space-y-1.5">
      ...
      <div className="relative">
        ...
        <input data-testid="input-password" type={show ? "text" : "password"} value={value} onChange={e => set(e.target.value)}
          ...
          className="w-full border-2 border-gray-200 rounded-xl pl-10 pr-12 py-3 text-gray-900 bg-gray-50 text-sm focus:outline-none focus:border-primary focus:bg-white transition-all" />
