...
import { Wallet, ArrowDownLeft, ArrowUpRight, Plus, TrendingUp, RefreshCw, Loader2, ArrowLeft, CheckCircle2, ExternalLink } from "lucide-react";
    ...
    <div className="app-shell pb-20 page-enter">
      ...
      <div className="px-4 pt-2 pb-4">
        ...
        <div className="relative rounded-[20px] overflow-hidden h-52 select-none"
          style={{ background: "linear-gradient(135deg, #0c1445 0%, #1a2f6e 40%, #1e3a8a 70%, #1d4ed8 100%)" }}>
          <div className="absolute inset-0 opacity-5"
            style={{ backgroundImage: "radial-gradient(circle at 70% 30%, white 1.5px, transparent 1.5px)", backgroundSize: "24px 24px" }} />
          ...
          <div className="absolute inset-0 p-5 flex flex-col justify-between">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-white font-bold text-sm">Investor Wallet</p>
              </div>
            ...
            <div className="text-center">
              ...
              {isLoading
                ...
                : <motion.p key={balance} initial={{ scale: 0.95 }} animate={{ scale: 1 }}
                    className="text-white font-bold text-3xl">{formatKES(balance)}</motion.p>}
            </div>
            <div className="flex items-end justify-between">
              <div>
                <p className="text-white/50 text-[9px] uppercase tracking-wider">{user?.name ?? "Cardholder"}</p>
              ...
              <div key={tx.id} className="flex items-center gap-3 bg-card border border-border rounded-xl p-3 mb-2">
                ...
                <div className="text-right flex-shrink-0">
                  <p className={`font-bold text-sm ${isCredit ? "text-green-600" : "text-red-500"}`}>
                    {isCredit ? "+" : "-"}{formatKES(parseFloat(tx.amount))}
                  </p>
                  ...
                  {initPaystack.isError ? (
                    <div className="space-y-3">
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-red-600 text-xs mt-1 leading-relaxed">
                              {(initPaystack.error as Error).message.includes("502") || (initPaystack.error as Error).message.includes("gateway")
                                ? "Paystack is temporarily unavailable. This may be a configuration issue. Please try again later."
                      ...
                      <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4">
                        ...
                        <ul className="text-amber-700 text-xs space-y-1.5 leading-relaxed">
                          <li>• M-Pesa send money: <strong>0700 000 000</strong> — acc: <strong>InvestaFarm</strong></li>
                          <li>• Bank transfer: Equity Bank — <strong>0140xxxxx</strong></li>
                          <li>• Email receipt to: <strong>support@investafarm.co.ke</strong></li>
                        </ul>
                      </div>
                      <button
                        onClick={() => initPaystack.reset()}
                        className="w-full border border-border text-foreground font-semibold py-3 rounded-xl flex items-center justify-center gap-2 text-sm active:scale-95"
                      >
                        ↩ Try Again
                      </button>
                    </div>
                    ...
                    <>
                      <div className="bg-blue-50 border border-blue-200 rounded-xl p-3">
                        <p className="text-blue-700 text-xs">Pay securely via <strong>Paystack</strong> — M-Pesa, Visa, Mastercard, or bank transfer. Funds credited instantly.</p>
                      </div>
                      <div>
              ...
              {modal === "paystack" && paystackRef && (
                <div className="space-y-4">
                  <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 text-center">
                    ...
                    <p className="text-blue-600 text-xs mt-1">Complete your payment, then click below to confirm.</p>
                  ...
                  <button onClick={() => window.open(data?.wallet ? `https://checkout.paystack.com/${paystackRef}` : "#", "_blank")}
