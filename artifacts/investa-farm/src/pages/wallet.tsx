...
export default function InvestorWallet() {
  ...
  function onPinSetupDone() {
    ...
    setHasPin(true);
    // If they came from an action, proceed to PIN gate
  ...
  return (
    <div className="app-shell pb-20 page-enter">
      ...
      <div className="px-4 pt-2 pb-4">
              ...
              <div>
                ...
                {stellarAcct?.accountNumber && (
                  <button onClick={handleCopyStellar} className="flex items-center gap-1 mt-1 group" aria-label="Copy account number">
                    ...
                    {stellarCopied
                      ? <Check size={8} className="text-green-400 ml-0.5" />
                      : <Copy size={8} className="text-white/40 group-hover:text-white/70 ml-0.5 transition-colors" />}
                  </button>
                )}
              </div>
        ...
        {/* PIN setup prompt for users without a PIN */}
        {hasPin === false && (
        ...
        {/* Add Funds + Withdraw — 2-column primary row */}
        ...
        {/* Finance — secondary row */}
        <button
          onClick={() => isFarmer ? setLocation("/farmer/loan-apply") : setModal("finance")}
          className="w-full mt-2 bg-blue-50 border border-blue-200 text-blue-700 font-bold py-3 rounded-2xl flex items-center justify-center gap-2 active:scale-95 transition-transform"
        >
          <TrendingUp size={15} />
          <span className="text-sm">Finance & Invest</span>
        </button>
      </div>
