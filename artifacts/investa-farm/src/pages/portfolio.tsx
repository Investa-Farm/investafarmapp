...
function buildChartData(
  ...
  totalValue: number,
  period: Period,
  realHistory?: { label: string; value: number }[],
) {
  // For 1W use the real 7-day wallet-balance history from the backend when available.
  // Fill any zero-value days (no transaction that day) with the previous known balance
  // so the line never collapses to zero.
  if (period === "1W" && realHistory && realHistory.length === 7) {
    let prev = totalInvested;
    return realHistory.map(p => {
      const val = p.value > 0 ? p.value : prev;
      prev = val;
      return { label: p.label, value: Math.max(val, totalInvested * 0.01) };
    });
  }

  // For 1M / 3M interpolate between totalInvested and totalValue — real daily
  // snapshots aren't stored beyond the last 7 days.
  const points = period === "1M" ? 30 : 90;
  const labels: string[] = [];
  const now = new Date();
  for (let i = points - 1; i >= 0; i--) {
    const d = new Date(now);
    d.setDate(d.getDate() - i);
    if (period === "1M") labels.push(d.getDate() === 1 || i === points - 1 || i === 0 ? d.toLocaleDateString("en-KE", { month: "short", day: "numeric" }) : "");
...
export default function Portfolio() {
  const { data: holdings, isLoading, isError: holdingsError } = useGetPortfolio();
  ...
  useScrollLock(wizardOpen || reinvestOpen || brokerUnlockOpen);
  ...
  const chartData = useMemo(() => {
    if (!summary) return [];
    return buildChartData(summary.totalInvested, summary.totalValue, period, summary.priceHistory);
  }, [summary, period]);
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center text-center bg-card border border-border rounded-2xl p-8 shadow-sm gap-4">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground text-xs mt-1 leading-relaxed">Check your connection and try refreshing the page.</p>
                </div>
                <button
                  onClick={() => window.location.reload()}
                  className="bg-primary text-white font-bold px-5 py-2.5 rounded-xl text-sm active:scale-95 transition-transform"
                >
                  Retry
                </button>
              </div>
