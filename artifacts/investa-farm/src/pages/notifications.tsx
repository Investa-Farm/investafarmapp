...
export default function NotificationsPage() {
  ...
  const isDemoUser = typeof window !== "undefined" &&
    !!sessionStorage.getItem("investa_token")?.includes("john.farmer") === false &&
    (() => { try { const u = JSON.parse(sessionStorage.getItem("investa_user") ?? "{}"); return u.email?.endsWith("investafarm.com") && (u.email?.startsWith("david") || u.email?.startsWith("john")); } catch { return false; } })();
  const merged: Notif[] = [
    ...apiNotifs,
  ...
  // Demo-account notifications (ids >= 9000) are client-only fixtures, never
  // persisted on the backend — hide them locally instead of calling DELETE.
  const deleteOne = (id: number) => {
    setLocalDeleted(prev => new Set([...prev, id]));
    if (id < 9000) deleteMut.mutate(id);
  };
