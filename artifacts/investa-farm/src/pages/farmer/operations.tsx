                ...
                return (
                  <>
                    {userCoords ? (
                      ...
                      <div className="flex items-center gap-1.5 text-[10px] text-muted-foreground bg-muted px-2.5 py-1.5 rounded-lg">
                        <MapPin size={10} /> Listed alphabetically — enable location for nearest suppliers
                      </div>
                    ) : null}
                    {sorted.map((s: any) => {
    ...
    <div className="app-shell pb-20 page-enter" data-testid="farmer-operations">
      ...
      <div className="px-4 pt-4 space-y-4">
        ...
        <button
          ...
          className="w-full flex items-center gap-3 bg-card border border-border rounded-2xl px-4 py-3.5 active:scale-[0.98] transition-all shadow-sm"
        >
          <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center flex-shrink-0">
            <BarChart2 size={18} className="text-primary" />
          </div>
          <div className="flex-1 text-left">
            <p className="text-foreground font-semibold text-sm">My Farms</p>
            <p className="text-muted-foreground text-[11px]">Listed farms, performance & investor activity</p>
          </div>
        ...
        {loans.length > 0 && (
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-primary/10 text-primary">{loans.length}</span>
            </div>
            <div className="space-y-2">
              {(loans as any[]).map((loan: any) => {
                const statusConfig: Record<string, { label: string; cls: string }> = {
                  ...
                  repaid:       { label: "Repaid",       cls: "bg-slate-100 text-slate-600" },
                };
                const cfg = statusConfig[loan.status as string] ?? { label: loan.status ?? "Unknown", cls: "bg-gray-100 text-gray-700" };
                return (
                  <div key={loan.id} className="bg-card border border-border rounded-2xl p-3.5 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap mb-0.5">
                        <p className="text-foreground font-semibold text-sm truncate">
                          {loan.cropName ?? "Farm Loan"}
                        </p>
                        <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full flex-shrink-0 ${cfg.cls}`}>{cfg.label}</span>
                      </div>
                      <p className="text-muted-foreground text-xs">{formatKES(loan.amount)} · {loan.farmLocation ?? "Kenya"}</p>
                      <p className="text-muted-foreground text-[10px] mt-0.5">
                        {loan.submittedAt
                          ? new Date(loan.submittedAt).toLocaleDateString("en-KE")
                          : loan.createdAt ? new Date(loan.createdAt).toLocaleDateString("en-KE") : ""}
                      </p>
                    </div>
                  </div>
                );
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border border-primary/30 text-primary text-xs font-semibold active:scale-95 transition-transform">
              <Plus size={13} /> Apply for More Funding
            </button>
          </div>
        )}
