...
export default function CropProposal() {
  ...
  // Crop-specific return multipliers based on Kenya market averages
  const CROP_RETURN_MULTIPLIERS: Record<string, number> = {
    maize: 1.9, wheat: 1.8, rice: 2.2, coffee: 3.1, tea: 2.7,
    avocado: 2.5, tomatoes: 2.0, beans: 1.8, soya: 1.9,
    sugarcane: 2.0, potatoes: 2.1, sunflower: 1.9, sorghum: 1.8,
    cassava: 1.7, millet: 1.7, horticulture: 2.3, dairy: 2.4,
  };
  const fundingAmt = parseFloat(form.fundingAmount) || 0;
  const cropKey = form.cropType?.toLowerCase().trim() ?? "";
  const returnMultiplier = CROP_RETURN_MULTIPLIERS[cropKey] ?? 2.1;
  const projectedRevenue = Math.round(fundingAmt * returnMultiplier);
  ...
  return (
    <div className="app-shell pb-20 page-enter">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <h1 className="text-white font-black text-xl">
              {mode === "form"
                ? (step === 1 ? "Farm Details" : step === 2 ? "Yield & Funding" : "Revenue Projection")
                : "Upload Proposal"}
            </h1>
          </div>
          {mode === "form" && (
            <div className="flex flex-col items-end gap-1">
              <p className="text-white/50 text-[9px] uppercase tracking-wider">Step {step} of 3</p>
              <div className="flex items-center gap-1">
                {[1, 2, 3].map(s => (
                  <div key={s} className={`h-1.5 rounded-full transition-all ${s <= step ? "bg-white w-7" : "bg-white/25 w-3"}`} />
                ))}
              </div>
            </div>
          )}
      ...
      <div className="px-4 pt-4 space-y-4">
        {/* Mode selector — choose how to apply */}
        <div className="grid grid-cols-2 gap-3">
          <button
            onClick={() => { setMode("form"); setStep(1); }}
            className={`flex flex-col items-center gap-2.5 p-4 rounded-2xl border-2 transition-all active:scale-95 ${mode === "form" ? "border-primary bg-primary/8 shadow-sm" : "border-border bg-card"}`}
          >
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${mode === "form" ? "bg-primary/15" : "bg-muted"}`}>
              <ClipboardList size={20} className={mode === "form" ? "text-primary" : "text-muted-foreground"} />
            </div>
            <div className="text-center">
              <p className={`text-xs font-bold ${mode === "form" ? "text-primary" : "text-foreground"}`}>Fill Form</p>
              <p className={`text-[9px] mt-0.5 leading-tight ${mode === "form" ? "text-primary/70" : "text-muted-foreground"}`}>3-step guided form</p>
            </div>
            {mode === "form" && <div className="w-1.5 h-1.5 rounded-full bg-primary" />}
          </button>
          <button
            onClick={() => { setMode("template"); }}
            className={`flex flex-col items-center gap-2.5 p-4 rounded-2xl border-2 transition-all active:scale-95 ${mode === "template" ? "border-primary bg-primary/8 shadow-sm" : "border-border bg-card"}`}
          >
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${mode === "template" ? "bg-primary/15" : "bg-muted"}`}>
              <Upload size={20} className={mode === "template" ? "text-primary" : "text-muted-foreground"} />
            </div>
            <div className="text-center">
              <p className={`text-xs font-bold ${mode === "template" ? "text-primary" : "text-foreground"}`}>Upload Document</p>
              <p className={`text-[9px] mt-0.5 leading-tight ${mode === "template" ? "text-primary/70" : "text-muted-foreground"}`}>PDF, Word or image</p>
            </div>
            {mode === "template" && <div className="w-1.5 h-1.5 rounded-full bg-primary" />}
          </button>
        </div>
              ...
              {step === 3 && (
                <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                  ...
                  <div className="bg-white rounded-2xl border border-border shadow-sm p-4">
                    ...
                    <p className="text-muted-foreground text-[10px] mb-4">Based on {returnMultiplier.toFixed(1)}× avg return for {form.cropType || "selected crop"} in Kenya</p>
