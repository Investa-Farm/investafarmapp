...
export default function FarmerProfile() {
  ...
  const hasDisbursed = loans.some((l: any) => l.status === "disbursed");

  const loanItems = [
    ...(activeLoans.length > 0 ? [{
      icon: DollarSign,
      label: "Repay Loan",
      sublabel: `${activeLoans.length} active loan${activeLoans.length !== 1 ? "s" : ""} · tap to make a repayment`,
      action: () => setLocation("/farmer/loan-apply"),
    }] : []),
    ...(hasDisbursed ? [{
      icon: Package,
      label: "Order Agri-Inputs",
      sublabel: "Seeds, fertilizer & tools — delivered to your farm",
      action: () => setLocation("/farmer/vouchers"),
    }] : []),
  ];
