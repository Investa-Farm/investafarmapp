          ...
          {showForm && (
            <motion.form initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
              ...
              <div className="relative">
                <textarea data-testid="input-update-description" value={description} onChange={e => setDescription(e.target.value.slice(0, 500))}
                  placeholder="Describe what happened on the farm..." required rows={3} maxLength={500}
                ...
                <span className={`absolute bottom-2 right-3 text-[10px] ${description.length >= 450 ? "text-amber-500" : "text-muted-foreground"}`}>
