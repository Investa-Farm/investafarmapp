...
const TX_ICONS: Record<string, { icon: React.ReactNode }> = {
  deposit:    { icon: <ArrowDownLeft size={16} /> },
  withdrawal: { icon: <ArrowUpRight size={16} /> },
  investment: { icon: <TrendingUp size={16} /> },
  return:     { icon: <Coins size={16} /> },
  fee:        { icon: <CreditCard size={16} /> },
  transfer:   { icon: <Repeat size={16} /> },
};

const QUICK_AMOUNTS = [1000, 5000, 10000, 25000, 50000];

const MPESA_CODES = [
  { code: "+254", name: "Kenya (Safaricom)" },
...
export function WalletModal({ open, onClose }: Props) {
  ...
  const deleteTxMutation = useMutation({
    mutationFn: async (id: number) => {
      const r = await fetch(`/api/wallet/transactions/${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
      if (!r.ok) throw new Error("Failed to delete transaction");
    },
    onSuccess: (_d, id) => {
      qc.setQueryData<WalletData>(["wallet"], (old) => old ? { ...old, transactions: old.transactions.filter(t => t.id !== id) } : old);
      setReceiptTx(null);
    },
  });
  ...
  const exportCsv = () => {
    downloadCsv(
      `investa-farm-wallet-${new Date().toISOString().slice(0, 10)}`,
      ["Date", "Type", "Description", "Amount", "Balance After", "Status", "Reference"],
      (data?.transactions ?? []).map(tx => [
        ...
        tx.balanceAfter,
        tx.status,
        tx.reference ?? `WL-${String(tx.id).padStart(6, "0")}`,
      ])
    );
  };
        ...
        >
          ...
          >
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {allTxs.length > 0 && (
                      <button onClick={exportCsv} className="flex items-center gap-1 text-[10px] font-bold text-primary bg-primary/10 px-2 py-1 rounded-full active:scale-95 transition-transform">
                ...
                {isLoading
                  ...
                  : filteredTxs.map(tx => {
                    const cfg = TX_ICONS[tx.type] ?? { icon: <CreditCard size={16} /> };
                    const isCredit = ["deposit", "return", "transfer"].includes(tx.type);
                    return (
                      <div key={tx.id}
                        className="w-full flex items-center gap-3 bg-card border border-border rounded-xl p-3 mb-2 transition-transform text-left">
                        <button onClick={() => setReceiptTx(tx)} className="flex items-center gap-3 flex-1 min-w-0 text-left active:scale-[0.98] transition-transform">
                          <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${isCredit ? "bg-green-500/10 text-green-600" : "bg-red-500/10 text-red-600"}`}>
                            {cfg.icon}
                          </div>
                        ...
                        <div className="text-right flex-shrink-0 flex items-center gap-2">
                          ...
                          <button
                            onClick={() => { if (confirm("Remove this transaction from your history?")) deleteTxMutation.mutate(tx.id); }}
            ...
            <AnimatePresence>
              {modal === "withdraw" && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  className="fixed inset-0 z-[61] flex items-end justify-center bg-black/50"
                  onClick={(e) => { if (e.target === e.currentTarget) setModal(null); }}>
                  <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }}
                    transition={{ type: "spring", damping: 30, stiffness: 300 }}
                    className="w-full bg-white rounded-t-3xl border-t-4 border-primary flex flex-col"
                    ...
                    {/* Sticky footer — submit button always stays visible above the keyboard/fold */}
                    <div className="px-5 py-4 border-t border-border bg-white flex-shrink-0">
                      {withdrawTab === "mpesa" && (
                        <button type="button" disabled={withdrawMutation.isPending || !amount || !phone.trim()}
                          onClick={() => {
            ...
            <AnimatePresence>
              {receiptTx && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  className="fixed inset-0 z-[62] flex items-end justify-center bg-black/60 backdrop-blur-sm"
                  ...
                  <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }}
                    ...
                    <div className="bg-gradient-to-br from-[#052e16] via-[#14532d] to-[#16a34a] px-5 pt-5 pb-8 relative overflow-hidden">
                      ...
                      <div className="text-center relative">
                        <div className={`w-12 h-12 rounded-2xl mx-auto mb-2 flex items-center justify-center ${["deposit","return","transfer"].includes(receiptTx.type) ? "bg-green-400/20 text-green-300" : "bg-red-400/20 text-red-300"}`}>
                          {TX_ICONS[receiptTx.type]?.icon ?? <CreditCard size={20} />}
                        </div>
                    ...
                    <div className="px-5 pb-8 space-y-2">
                      ...
                      <button
                        onClick={() => { if (confirm("Remove this transaction from your history?")) deleteTxMutation.mutate(receiptTx.id); }}
                        disabled={deleteTxMutation.isPending}
                        className="w-full py-3 rounded-2xl border-2 border-red-100 text-red-500 font-bold text-sm active:scale-95 transition-transform flex items-center justify-center gap-1.5 disabled:opacity-50">
                        <Trash2 size={14} /> {deleteTxMutation.isPending ? "Removing…" : "Delete from history"}
                      </button>
                    </div>
                  </motion.div>
                </motion.div>
