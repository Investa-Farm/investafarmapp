...
import { db, primaryDb, shadowDb, kycDocumentsTable, usersTable, notificationsTable } from "@workspace/db";
...
router.get("/kyc/admin/pending", async (req, res): Promise<void> => {
  ...
  // Query the fixed primary DB (always Supabase when SUPABASE_DATABASE_URL is set,
  // always Neon otherwise — never the smart failover handle which could point to either)
  const primaryDocs = await primaryDb
    .select(selectFields)
    .from(kycDocumentsTable)
    .innerJoin(usersTable, eq(kycDocumentsTable.userId, usersTable.id))
    .orderBy(kycDocumentsTable.createdAt);

  // Also query the shadow/fallback DB (Neon or Supabase depending on config)
  // to surface docs that may have been submitted when the other DB was active
  let shadowDocs: typeof primaryDocs = [];
  if (shadowDb) {
    try {
      shadowDocs = await shadowDb
        .select(selectFields)
        .from(kycDocumentsTable)
        .innerJoin(usersTable, eq(kycDocumentsTable.userId, usersTable.id))
        .orderBy(kycDocumentsTable.createdAt);
    } catch {
      // Shadow DB unavailable — continue with primary results only
    }
  }

  // Merge results from both DBs and deduplicate by fileUrl (the actual uploaded
  // document URL is globally unique regardless of which DB assigned which numeric id).
  // If a doc was dual-written it will appear in both sets with the same fileUrl;
  // if it was only written to one DB it will appear once.  We keep the primary-DB
  // version when both sets contain the same fileUrl (primary data is authoritative).
  const seenFileUrl = new Set<string | null>();
  const merged: typeof primaryDocs = [];

  // Add primary docs first (authoritative)
  for (const doc of primaryDocs) {
    seenFileUrl.add(doc.fileUrl);
    merged.push(doc);
  }
  // Add shadow docs only when their fileUrl hasn't already been seen
  for (const doc of shadowDocs) {
    if (!seenFileUrl.has(doc.fileUrl)) {
      seenFileUrl.add(doc.fileUrl);
      merged.push(doc);
    }
  }
