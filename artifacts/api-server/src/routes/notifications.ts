...
// DELETE /notifications/:id — permanently removes a notification/message
// (including admin messages, which surface to the user as notifications).
router.delete("/notifications/:id", async (req, res): Promise<void> => {
  const user = await getCurrentUser(req);
  if (!user) { res.status(401).json({ error: "Unauthorized" }); return; }
  const id = parseInt(req.params["id"] ?? "", 10);
  if (!id) { res.status(400).json({ error: "Invalid notification id" }); return; }
  const [row] = await db.select({ id: notificationsTable.id }).from(notificationsTable)
