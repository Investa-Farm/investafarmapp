...
async function autoCreateFarmAndListing(
  ...
  // Pricing engine v2: risk score + explainability (Groq, with deterministic
  // fallback), confidence-interval revenue forecast, cold-start capping, and
  // a human-review flag — computed once at listing time and stored on the farm.
  const farmerHarvestCount = (await db.select().from(farmsTable)
    .where(and(eq(farmsTable.farmerId, farmerId), eq(farmsTable.status, "harvested")))).length;

  let v2: Awaited<ReturnType<typeof computePrimaryPricingV2>> | undefined;
  try {
    v2 = await computePrimaryPricingV2({
      capitalNeededKes: amount,
      crop: cropType,
      landSizeHa: 1,
    ...
    console.error("[autoCreateFarmAndListing] pricing v2 failed, proceeding without explainability:", (e as Error).message);
  }
