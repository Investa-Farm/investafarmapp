...
import { sendInvestmentSms, sendFarmerCreditSms, sendFarmFullyFundedSms, sendVoucherSms } from "../lib/sms";
...
router.post("/market/buy", financialRateLimit, requireNonce, async (req, res): Promise<void> => {
  ...
  // Keep the listing active (visible in the market) even when fully sold out —
  // the frontend shows a "Fully Funded" state instead of hiding the card.
  await db.update(marketListingsTable)
    .set({ sharesAvailable: Math.max(0, listing.sharesAvailable - quantity) })
  ...
  // Notify investor (in-app + push + SMS)
  notifyUser(
    user.id,
    "investment_made",
  ...
  if (user.phone) {
    sendInvestmentSms(user.phone, farm?.name ?? "a farm", quantity, totalAmount).catch(() => {});
  }
  ...
  if (isPrimary && farm) {
    ...
    db.select({ phone: usersTable.phone, name: usersTable.name })
      .from(usersTable).where(eq(usersTable.id, farm.farmerId)).limit(1)
      .then(([f]) => {
        if (f?.phone) sendFarmerCreditSms(f.phone, f.name, totalAmount, `investment in ${farm.name}`).catch(() => {});
      }).catch(() => {});
    ...
    try {
      ...
      if (loan && loan.status !== "disbursed") {
        ...
        if (farmer) {
          ...
          if (farmer.phone) {
            sendFarmFullyFundedSms(farmer.phone, farmer.name, farm?.name ?? "Your Farm", lAmt).catch(() => {});
            sendVoucherSms(farmer.phone, vCode, farm?.name ?? "Your Farm").catch(() => {});
          }
