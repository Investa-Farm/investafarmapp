...
router.post("/support/tickets", async (req, res): Promise<void> => {
  ...
  let ticket: any;
  try {
    const [t] = await db.insert(supportTicketsTable).values({
      userId: user.id,
      category: cat,
    ...
    console.error("Support ticket DB error:", err);
    res.status(500).json({ error: "Could not save your ticket. Please try again or email us at investafarm@proton.me" });
    return;
  }
