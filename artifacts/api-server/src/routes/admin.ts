...
import { sendPushToUser, createInAppNotification, notifyUser } from "../lib/push";
...
router.patch("/admin/farms/:id/status", async (req, res): Promise<void> => {
  ...
  // Auto-disburse loan funds to farmer's wallet when farm is approved (goes active)
  if (status === "active") {
    try {
      ...
      if (farm) {
        const [loan] = await db
          .select()
          .from(loanApplicationsTable)
          .where(and(eq(loanApplicationsTable.farmId, id), eq(loanApplicationsTable.status, "approved")))
          .limit(1);
        if (loan) {
          const amount = Number(loan.amount);
          const ref = `LOAN-DISBURSE-${loan.id}-${Date.now()}`;
          await creditWallet(loan.farmerId, amount, {
            type: "deposit",
            description: `Loan disbursement — ${farm.name}`,
            reference: ref,
          });
          const statusHistory = Array.isArray(loan.statusHistory) ? [...loan.statusHistory] : [];
          statusHistory.push({ stage: "disbursed", at: new Date().toISOString(), note: "Auto-disbursed on farm approval" });
          await db
            .update(loanApplicationsTable)
            .set({ status: "disbursed", statusHistory })
            .where(eq(loanApplicationsTable.id, loan.id));
          notifyUser(loan.farmerId, "wallet_credit", "💰 Loan Disbursed!", `KES ${amount.toLocaleString("en-KE")} has been credited to your wallet for ${farm.name}.`, "/wallet").catch(() => {});
        }
      }
    } catch (e) {
      // Non-fatal — farm status was already updated; log but don't fail the response
      console.error("[admin/farms/status] Auto-disburse failed:", (e as Error).message);
    }
  }
...
// ── (bulk demo seeder removed — production only) ─────────────────────────────
// POST /admin/seed-bulk-demo — removed; use the DB seed script directly
router.post("/admin/seed-bulk-demo", async (_req, res): Promise<void> => {
  res.status(410).json({ error: "Bulk demo seeder has been removed from production." });
});
