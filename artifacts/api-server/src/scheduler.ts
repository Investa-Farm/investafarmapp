...
import { getRainfallData, getKenyaCoords, checkRainfallAlerts, computeRainfallFactor } from "./lib/rainfall";
...
/** Pick a variant deterministically per user (same message for same user across runs) */
function pickForUser<T>(arr: readonly T[], userId: number): T {
  return arr[Math.abs(userId) % arr.length] as T;
}

// ── Notification variant pools ─────────────────────────────────────────────────
const PRICE_ALERT_BODIES: ReadonlyArray<(name: string, pct: string, price: string) => string> = [
  (n, pct, p) => `${n} moved ${pct}% to KES ${p}`,
  (n, pct, p) => `Your ${n} holding shifted ${pct}%. Current price: KES ${p} per share.`,
  (n, pct, p) => `Market update: ${n} is now at KES ${p} (${pct}% change). Review your position.`,
  (n, pct, p) => `Alert: ${n} changed ${pct}% — now KES ${p}. Tap to check your portfolio.`,
];

const DIVIDEND_TITLES: ReadonlyArray<() => string> = [
  () => "💰 Harvest Payout Received!",
  () => "🌾 Your farm paid out!",
  () => "✅ Dividend credited to wallet",
  () => "🎉 Harvest complete — funds arrived!",
];
const DIVIDEND_BODIES: ReadonlyArray<(amount: string, farm: string, shares: number) => string> = [
  (a, f, s) => `KES ${a} credited from ${f} (${s} shares). Check your wallet.`,
  (a, f, s) => `Your harvest from ${f} is here! KES ${a} added to your wallet for ${s} shares.`,
  (a, f, s) => `Payday! ${f} harvest — ${s} shares × dividend = KES ${a} now in your wallet.`,
  (a, f, s) => `${f} harvest complete. KES ${a} from ${s} shares credited to your account.`,
];

const VERIFICATION_BODIES: ReadonlyArray<(days: string) => string> = [
  (d) => `Your account has been unverified for ${d} days. Check your inbox to activate it.`,
  (d) => `You're one step away — verify your email to unlock all Investa Farm features (${d} days pending).`,
  (d) => `Reminder: verify your email to access the full platform. It's been ${d} days since sign-up.`,
  (d) => `Email verification still pending (${d} days). Check your inbox to complete setup.`,
];

const LOAN_OVERDUE_BODIES: ReadonlyArray<(label: string, amount: string) => string> = [
  (l, a) => `Your ${l} of KES ${a} is overdue. Pay now from your wallet to avoid disruption.`,
  (l, a) => `Urgent: ${l} repayment of KES ${a} is past due. Settle from your wallet today.`,
  (l, a) => `KES ${a} for your ${l} is overdue. Tap to pay from your wallet and stay on track.`,
];
const LOAN_DUE_BODIES: ReadonlyArray<(label: string, amount: string, date: string) => string> = [
  (l, a, d) => `Your ${l} installment of KES ${a} is due ${d}. Tap to pay from your wallet.`,
  (l, a, d) => `Heads up: ${l} repayment of KES ${a} due ${d}. Ensure your wallet is funded.`,
  (l, a, d) => `Repayment reminder: KES ${a} for ${l} due ${d}. Pay early to stay current.`,
];

const ORDER_BUY_BODIES: ReadonlyArray<(qty: string, farm: string, price: string) => string> = [
  (q, f, p) => `${q} shares of ${f} @ KES ${p} — funds deducted from wallet.`,
  (q, f, p) => `Buy order filled! ${q} shares of ${f} at KES ${p} each. Check your portfolio.`,
  (q, f, p) => `You now own ${q} more shares of ${f} @ KES ${p}. View your updated portfolio.`,
];
const ORDER_SELL_BODIES: ReadonlyArray<(qty: string, farm: string, total: string) => string> = [
  (q, f, t) => `${q} shares of ${f} sold — KES ${t} added to wallet.`,
  (q, f, t) => `Sell order filled! ${q} shares of ${f} sold. KES ${t} credited to your wallet.`,
  (q, f, t) => `Done! ${q} ${f} shares sold. KES ${t} is now in your wallet.`,
];
...
export function startScheduler(): void {
  ...
  cron.schedule("* * * * *", () => runOrderMatching(), { timezone: "Africa/Nairobi" });
  ...
  console.log("[scheduler] Order matching engine: every 1 minute");
...
async function runVerificationReminders(): Promise<void> {
  try {
    ...
    for (const user of allUnverified) {
      ...
      const days = String(Math.round(daysSince));
      notifyUser(
        user.id,
        "general",
        "⚠️ Verify your email",
        pickForUser(VERIFICATION_BODIES, user.id)(days),
        "/verify-otp"
      ).catch(() => {});
      sent++;
    }
...
async function runPriceSimulation(): Promise<void> {
  try {
    ...
    // ── Bulk-fetch news/AI sentiment scores for all crop types ─────────────────
    // sentimentFactor = 1 + 0.03 × ((positivePct − negativePct) / 100)
    // Range: ~0.87–1.13  (neutral = 1.0 when positivePct = negativePct)
    const sentimentRows = await db.select().from(sentimentScoresTable).catch(() => []);
    ...
    for (const row of sentimentRows) {
      ...
      // Keep the most-recent entry per crop (rows already ordered by createdAt desc naturally)
      if (!sentimentMap.has(row.cropType)) {
    ...
    for (const farm of farms) {
      ...
      const S_base    = CROP_RISK_S[cropKey] ?? 5;   // baseline before rainfall
      let S           = S_base;
      ...
      // ── Rainfall cascade: adjust S based on live weather data ──────────────
      let rainfallFactor = 1.0;
      let rainfallTotalMm = 0;
      try {
        const [lat, lng] = getKenyaCoords(farm.location ?? "");
        const rainfall = await getRainfallData(lat, lng, cropKey);
        rainfallFactor = rainfall.rainfallFactor;
        rainfallTotalMm = rainfall.seasonalTotalMm;
        // Cascade rainfall into risk score:
        //   drought (<70% of optimal min) → S –3 to –5
        //   low rain (70–100% of min)     → S –1 to –2
        //   excess rain                    → S –1 to –3
        if (rainfall.riskLevel === "drought") {
          const severityPct = rainfallFactor;
          S = Math.max(1, S - Math.round((1 - severityPct) * 5));
        } else if (rainfall.riskLevel === "low") {
          S = Math.max(1, S - 1);
        } else if (rainfall.riskLevel === "excess") {
          const floodPenalty = rainfall.floodRisk ? 3 : 1;
          S = Math.max(1, S - floodPenalty);
        }
        if ((farm as any).riskScore !== S) {
          await db.update(farmsTable)
            .set({ riskScore: S } as any)
            .where(eq(farmsTable.id, farm.id))
            .catch(() => {});
        }
      } catch { /* use baseline S if rainfall fetch fails */ }

      // ── News/AI sentiment factor ────────────────────────────────────────────
      ...
      //
      // d_baseline uses the crop's BASE risk (before rainfall) because it was
      // established at farm creation.  Rainfall & sentiment shift D̂(t) directly.
      //
      ...
      // ── Secondary market fair-value formula ─────────────────────────────────
      // P_sell(t) = (D̂(t) / timeDiscount(t)) × (1 − λ(t)) × imbalance × (1 − δ)
      //
      // At t=0:  P_sell ≈ P₀ × 0.98  (correct: tiny day-1 cost from liquidity)
      // As t→T:  P_sell rises naturally toward P₀ × (1+rf)^(T/365) × 0.98
      ...
      // Secondary market price with demand pressure and liquidity discount:
      // P_sell = P_fair × imbalance × (1 – δ)
      ...
      if (arbDeviation > ARB_THRESHOLD) {
        console.log(
          `[arb-alert] ${farm.name}: P_exec=${P_execution.toFixed(2)} P_time=${P_time.toFixed(2)} dev=${(arbDeviation * 100).toFixed(1)}% rf=${rainfallFactor.toFixed(2)} S=${S}`
        );
      }

      // Cap per-tick movement at ±10% to prevent extreme jumps
      ...
      // Log rainfall-driven price adjustments (drought/flood events)
      if (rainfallFactor < 0.85) {
        console.log(
          `[pricing] ${farm.name} (${cropKey}): rainfall=${Math.round(rainfallTotalMm)}mm factor=${rainfallFactor.toFixed(2)} S=${S} λ=${lambda.toFixed(3)} P_fair=${P_fair.toFixed(2)}`
        );
      }
    }
    console.log(`[scheduler] Dynamic pricing updated ${farms.length} farms (rainfall-adjusted)`);
  } catch (e) {
    ...
    for (const farm of farms) {
      ...
      if (Math.abs(changePct) >= threshold && now - prev.lastAlertedAt > 30 * 60 * 1000) {
        ...
        if (investorIds.length > 0) {
          ...
          const farmName = (farm as any).name as string;
          const pctStr = (changePct > 0 ? "+" : "") + changePct.toFixed(1);
          const priceStr = Math.round(currentPrice).toLocaleString();
          // Send per-investor variant so each user gets different wording
          await Promise.all(investorIds.map(id =>
            notifyUser(id, "price_alert",
              `${direction} Price Alert: ${farmName}`,
              pickForUser(PRICE_ALERT_BODIES, id)(farmName, pctStr, priceStr),
              `/market/${farm.id}`
            ).catch(() => {})
          ));
  ...
  try {
    ...
    for (const inv of dueInvestments) {
      try {
        ...
        // Notify investor with per-user variant wording
        notifyUser(
          inv.investorId,
          "dividend_paid",
          pickForUser(DIVIDEND_TITLES, inv.investorId)(),
          pickForUser(DIVIDEND_BODIES, inv.investorId)(roundedAmount.toLocaleString("en-KE"), farm.name, inv.quantity),
          "/portfolio"
        ).catch(() => {});
      ...
      for (const sell of sells) {
        ...
        for (const buy of sortedBuys) {
          ...
          notifyUser(buy.investorId, "order_filled",
            "✅ Buy Order Filled",
            pickForUser(ORDER_BUY_BODIES, buy.investorId)(fillQty.toFixed(2), farm.name, fillPrice.toLocaleString("en-KE")),
            "/portfolio"
          ).catch(() => {});

          notifyUser(sell.investorId, "order_filled",
            "✅ Sell Order Filled",
            pickForUser(ORDER_SELL_BODIES, sell.investorId)(fillQty.toFixed(2), farm.name, fillTotal.toLocaleString("en-KE")),
            "/portfolio"
          ).catch(() => {});
    ...
    for (const loan of loans) {
      try {
        ...
        const body = isOverdue
          ? pickForUser(LOAN_OVERDUE_BODIES, loan.farmerId)(farmLabel, remainingStr)
          : pickForUser(LOAN_DUE_BODIES, loan.farmerId)(farmLabel, remainingStr, dueAt.toLocaleDateString("en-KE"));
