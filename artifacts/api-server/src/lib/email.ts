...
// Secondary sender: fallback when Resend is unavailable or fails.
// Set BREVO_API_KEY. Optionally BREVO_FROM_EMAIL (verified sender in Brevo dashboard).
async function sendViaBrevo(opts: {
  ...
  html: string;
  replyTo?: string;
}): Promise<"sent" | "bounced" | "failed"> {
  const apiKey = process.env.BREVO_API_KEY;
  if (!apiKey) return "failed";

  // Extract plain email and display name from "Display Name <email@domain.com>" format
  const fromMatch = opts.from.match(/^"?([^"<]+)"?\s*<([^>]+)>$/) ?? opts.from.match(/^(.+)$/);
  const fromName  = fromMatch?.[2] ? fromMatch[1].replace(/^"|"$/g, "").trim() : APP_NAME;
  const fromEmail = fromMatch?.[2] ?? (fromMatch?.[1] ?? "");
  // Use BREVO_FROM_EMAIL if set, otherwise fall back to extracted email
  const senderEmail = process.env.BREVO_FROM_EMAIL ?? fromEmail;
  const senderName  = APP_NAME;
  ...
  try {
    ...
    if (resp.ok) {
      ...
      const msgId = (body as { messageId?: string }).messageId ?? "(no id)";
      console.info(`[EMAIL] Sent via Brevo: "${opts.subject}" → ${opts.to} (messageId: ${msgId})`);
      return "sent";
    }

    const errBody = await resp.text().catch(() => "");
    // 400 with "invalid_parameter" or bounce-class errors
    if (resp.status === 400 && errBody.toLowerCase().includes("blocked")) {
      console.warn(`[EMAIL] Brevo rejected (blocked/bounced) for ${opts.to}: ${errBody}`);
      return "bounced";
    }
    console.warn(`[EMAIL] Brevo delivery failed (${resp.status}): ${errBody}`);
    return "failed";
  } catch (err) {
    console.warn("[EMAIL] Brevo request error:", err instanceof Error ? err.message : String(err));
    return "failed";
  }
}
...
function createTransport() {
  ...
  if (!hasResend && !hasBrevo && !hasSmtp) {
    console.warn("[EMAIL] No email provider configured — set RESEND_API_KEY (primary), BREVO_API_KEY (fallback), or GOOGLE_SMTP_USER + GOOGLE_SMTP_PASS (last resort)");
    return null;
  }
  ...
  return {
    sendMail: async (opts: SendMailOpts) => {
      ...
      if (hasResend) {
        ...
        console.warn(`[EMAIL] Resend failed for "${opts.subject}" — trying Brevo fallback`);
      }

      // 2. Brevo fallback (HTTP — secondary provider, works everywhere)
      if (hasBrevo) {
        ...
        if (result === "bounced") {
          // Hard bounce — recipient address is blocked; no point retrying via SMTP
          console.warn(`[EMAIL] Brevo reports hard bounce for ${opts.to} — skipping SMTP retry`);
          return;
        }
        console.warn(`[EMAIL] Brevo failed for "${opts.subject}" — trying SMTP fallback`);
      }
...
export async function testSmtpConnection(): Promise<void> {
  ...
  if (hasResend) {
    console.info("[EMAIL] Resend API configured — primary email provider (HTTP)");
  }
  if (!!process.env.BREVO_API_KEY) {
    console.info("[EMAIL] Brevo API configured — will be used as fallback if Resend fails");
  }
  ...
  } else if (!hasResend && !process.env.BREVO_API_KEY) {
